...
var palindrome = function(string) {
    if (string.length <= 1) {
        ...
        return string[0].toLowerCase() === string[string.length - 1].toLowerCase() && palindrome(string.slice(1,string.length-1).trim());
    }
};
...
var countKeysInObj = function(obj, key) {
    var found = (obj[key] === undefined) ? 0 : 1;
    for (let search in obj) {
        if (typeof obj[search] === 'object') {
            found += countKeysInObj(obj[search],key);
        }
    }
    return found;
};
...
var countValuesInObj = function(obj, value) {
    ...
    for (let search in obj) {
        if (typeof obj[search] === 'object') {
            found += countValuesInObj(obj[search],value);
        } else if (obj[search] === value) {
            found += 1;
        }
    }
    return found;
...
var replaceKeysInObj = function(obj, oldKey, newKey) {
    var found = (obj[oldKey] === undefined) ? 0 : 1;
    ...
    for (let search in obj) {
        if (typeof obj[search] === 'object') {
            replaceKeysInObj(obj[search],oldKey,newKey);
        }
    }
    return obj;
};
...
var nestedEvenSum = function(obj) {
    ...
    for (let search in obj) {
        if (typeof obj[search] === 'object') {
            found += nestedEvenSum(obj[search]);
        } else if (obj[search] % 2 === 0) {
            found += obj[search];
        }
    }
    return found;
...
// their original sign. The first number in th e index always needs to be positive.
...
var tagCount = function(tag, node) {
    ...
    let curTag = node.tagName.toLowerCase();
    result[curTag] = 1;
    if (curTag === tag) {
        result += 1;
    }
    for (let index = 0 ; index < node.children.length ; index ++) {
        result += tagCount(tag, node.children[index]);
    }
    return result;
};
...
var mergeSort = function(array) {
      ...
      return mergeHelper(mergeSort(array.slice(0,mid)), mergeSort(array.slice(mid,array.length)))
};

var mergeHelper = function(leftArr, rightArr) {
    length = leftArr.length + rightArr.length;
    leftArr.push(Infinity);
    rightArr.push(Infinity);
    let result = [];
    for(let index = 0;index < length ; index ++){
        result[index] = (leftArr[0]>rightArr[0]) ? rightArr.shift() : leftArr.shift();
    }
    return result;
  }
